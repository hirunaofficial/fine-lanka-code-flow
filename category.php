<?php if(!isset($_GET['category'])){ header('Location:index.php');}else{$category = $_GET['category'];} ?>
<?php
$title = "Category";
include 'header.php'; 
?>
<?php 
$cat_places = $conn->query("SELECT * FROM places WHERE category='$category';");
$img_main = $conn->query("SELECT * FROM places WHERE category='$category' ORDER BY RAND();")->fetch_assoc()['img'];
?>
               ...
               <div class="inner-baner-container" style="background-image: url(<?php echo $img_main; ?>);">
                  <div class="container">
                     <div class="inner-banner-content">
                        <h1 class="inner-title">#<?php echo $category; ?></h1>
                     </div>
                  </div>
               </div>
